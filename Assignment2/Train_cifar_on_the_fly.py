...
lenght_x_train_without_validation = len(x_train_cifar10)
...
y_train_cifar10 = np_utils.to_categorical(y_train_cifar10, num_classes)

y_validation_cifar10 = np_utils.to_categorical(y_validation_cifar10, num_classes)
...
for i in range(0,n):
    ...
    autoencoder.save('autoencoder_plus_cifar'+str(i)+'.h5')
    classifier.save('classifier_plus_cifar' + str(i) + '.h5')
...
for i in range(0,n):
    ...
    filepath_autoencoder = "weights_autoencoder_plus_cifar.best.hdf5"
    ...
    autoencoder = load_model('autoencoder_plus_cifar'+str(i)+'.h5')
...
for k in range(0,epochs):
    ...
    for i in range(0,number_of_batches):
        ...
        if threshold_classifier_creation > np.min(score_for_autoencoder):
            ...
            autoencoder = load_model('autoencoder_plus_cifar' + str(min_error_index) + '.h5')
            classifier = load_model('classifier_plus_cifar' + str(min_error_index) + '.h5')
            index_increased = False
        else:
            print('Classifier was created')
            autoencoder, classifier = instantiate_autoencoder_and_classifier(input_shape)
            autoencoder.save('autoencoder_plus_cifar' + str(n) + '.h5')
            classifier.save('classifier_plus_cifar' + str(n) + '.h5')
            n = n+1
        ...
        if index_increased == True:
            ...
            autoencoder.save('autoencoder_plus_cifar' + str(min_error_index) + '.h5')
            classifier.save('classifier_plus_cifar' + str(min_error_index) + '.h5')
...
'''   
    for l in range(0, number_of_validation_batches):
        ...
        for j in range(0, n):
            autoencoder = load_model('autoencoder_plus_cifar_1_' + str(j) + '.h5')
            score_for_autoencoder.append(autoencoder.evaluate(test_set, test_set))

        min_error_index = np.argmin(score_for_autoencoder)
        classifier = load_model('classifier_plus_cifar_1_' + str(min_error_index) + '.h5')
